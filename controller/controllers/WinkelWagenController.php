...
class WinkelWagenController extends Controller
{
    ...
    public function bestel()
    {
        ...
        $tmp_bestelling = new Bestelling(null, $_SESSION['email'], $date, null, "besteld");
        if (! $this->checkBestellingOpDag($date, $_SESSION['email'])) {
            $this->bestellingMapper->add($tmp_bestelling);
            $error = false;
            foreach ($_POST['bestellingsLijn'] as $key => $bestellingsLijn) {
                if (! preg_match('/[0-9]/', $bestellingsLijn['aantal'])) {
                    ...
                    $b = new BestellingsLijn($this->bestellingMapper->getBestellingsIdForEmail($_SESSION['email'], $date), null, $key, $bestellingsLijn['aantal']);
                    
                    $this->bestellingsLijnMapper->add($b);
                }
            }
            if ($error) {
                $this->bestellingMapper->delete('bestelling', $this->bestellingMapper->getBestellingsIdForEmail($_SESSION['email'], $date));
                $_GET['message'] = 'Verkeerde waarde';
            } else {
